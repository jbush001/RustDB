...
pub const RA_HEADER_SIZE: usize = 32;
const ENTRY_START_FIELD_OFFS: usize = 32;
const NUM_ENTRIES_FIELD_OFFS: usize = 34;
const INDEX_OFFS: usize = 36;

pub fn init_record_array(page: &mut [u8]) {
    page.fill(0);
    set_u16(page, ENTRY_START_FIELD_OFFS, page.len() as u16);
}

pub fn get_ra_num_entries(page: &[u8]) -> usize {
    get_u16(page, NUM_ENTRIES_FIELD_OFFS) as usize
}

pub fn get_ra_free_space(page: &[u8]) -> usize {
    let index_end = INDEX_OFFS + get_ra_num_entries(page) * 2;
    let entry_start = get_u16(page, ENTRY_START_FIELD_OFFS) as usize;

    entry_start - index_end
}

pub fn insert_record_array(page: &mut [u8], index: usize, value: &[u8]) {
    assert!(get_ra_free_space(page) >= value.len(), "insert failed: insufficient space");

    let num_recs = get_ra_num_entries(page);
...
pub fn delete_record_array(page: &mut [u8], index: usize) {
    let total_recs = get_ra_num_entries(page);
    assert!(index < total_recs, "delete_record_array: invalid index");
...
mod tests {
    ...
    fn sanity_check_record_array(page: &[u8]) {
        ...
        let num_entries = get_ra_num_entries(page);
    ...
    fn test_sanity_check_offset_out_of_range() {
        ...
        insert_record_array(&mut page, 0, "aaaaa".as_bytes());
        insert_record_array(&mut page, 1, "bbbbb".as_bytes());
    ...
    fn test_sanity_incorrect_record_offset() {
        ...
        insert_record_array(&mut page, 0, "a".as_bytes());
        insert_record_array(&mut page, 1, "z".as_bytes());
    ...
    fn test_insert_after() {
        ...
        insert_record_array(&mut page, 0, &record1);

        let record2 = "bbbbb".as_bytes();
        insert_record_array(&mut page, 1, &record2);
    ...
    fn test_insert_before() {
        ...
        let record2 = "bbbbb".as_bytes();
        insert_record_array(&mut page, 0, &record2);
    ...
    fn test_insert_mid() {
        ...
        insert_record_array(&mut page, 0, &record0);
    ...
    fn test_free_space() {
        ...
        let record1 = "aaaa".as_bytes();
        let init_free_space = get_ra_free_space(&page);
        insert_record_array(&mut page, 0, &record1);
        assert_eq!(get_ra_free_space(&page), init_free_space - record1.len() - 4);

        let record2 = "abcdefghijklmnopqrstuvwxyz".as_bytes();
        let init_free_space = get_ra_free_space(&page);
        insert_record_array(&mut page, 1, &record2);
        assert_eq!(get_ra_free_space(&page), init_free_space - record2.len() - 4);
    ...
    fn test_insert_invalid_index() {
        ...
        insert_record_array(&mut page, 3, &record3);
    ...
    fn test_out_of_space() {
        ...
        for _ in 0..1024 {
            insert_record_array(&mut page, 0, "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb".as_bytes());
        }
    }
    ...
    fn test_delete_first() {
        ...
        let mut page: [u8; 4096] = [0; 4096];
        init_record_array(&mut page);
        insert_record_array(&mut page, 0, "apple".as_bytes());
        insert_record_array(&mut page, 0, "aardvark".as_bytes());
        insert_record_array(&mut page, 2, "banana".as_bytes());
        insert_record_array(&mut page, 3, "zebra".as_bytes());

        // Remove first entry (aardvark)
        delete_record_array(&mut page, 0);
        assert_eq!(get_ra_num_entries(&page), 3);
        sanity_check_record_array(&page);
    ...
    fn test_delete_middle() {
        ...
        delete_record_array(&mut page, 1);
        assert_eq!(get_ra_num_entries(&page), 3);
        sanity_check_record_array(&page);
    ...
    fn test_delete_last() {
        ...
        // Remove last entry (zebra)
        delete_record_array(&mut page, 3);
        assert_eq!(get_ra_num_entries(&page), 3);
        sanity_check_record_array(&page);
    ...
    fn test_remove_all() {
        let mut page: [u8; 4096] = [0; 4096];
        init_record_array(&mut page);
        let capacity = get_ra_free_space(&page);

        insert_record_array(&mut page, 0, "apple".as_bytes());
        insert_record_array(&mut page, 1, "aardvark".as_bytes());

        delete_record_array(&mut page, 0);
        delete_record_array(&mut page, 0);
        assert_eq!(get_ra_num_entries(&page), 0);
        sanity_check_record_array(&page);

        assert_eq!(capacity, get_ra_free_space(&page));
    }

    #[test]
    #[should_panic = "delete_record_array: invalid index"]
