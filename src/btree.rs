...
// 22..29 |  u64   | Right child. Value for nodes greater than max key
...
const RIGHT_CHILD_FIELD_OFFS: usize = 22;
...
fn get_entry_value(node: &[u8], rec_num: usize) -> u64 {
    ...
    if rec_num == num_entries {
        return get_u64(node, RIGHT_CHILD_FIELD_OFFS);
    }
...
fn split_node(orig: &[u8], out1: &mut [u8], out2: &mut [u8]) -> Vec<u8> {
    ...
    if !is_leaf(orig) {
        ...
        set_u64(out1, RIGHT_CHILD_FIELD_OFFS, get_entry_value(orig, orig_index));
        orig_index += 1;
    }
    ...
    set_u64(out2, RIGHT_CHILD_FIELD_OFFS, get_u64(orig, RIGHT_CHILD_FIELD_OFFS));
...
mod tests {
    ...
    fn test_get_key_val() {
        let mut node: [u8; 4096] = [0; 4096];
        super::init_node(&mut node);
        set_u64(&mut node, super::RIGHT_CHILD_FIELD_OFFS, 0x1111222233334444);
        ...
        // Asking for the last entry should return the right child value.
        assert_eq!(super::get_entry_value(&node, 2), 0x1111222233334444);
    }
    ...
    fn test_get_free_space() {
        let mut node: [u8; 4096] = [0; 4096];
        super::init_node(&mut node);
        assert_eq!(super::get_node_free_space(&node), 4096 - super::INDEX_OFFS);
        super::insert_entry(&mut node, "aardvark".as_bytes(), 1000);
        assert_eq!(super::get_node_free_space(&node), 4096 - super::INDEX_OFFS - 20);
    }
    ...
    fn test_split_interior_node() {
        ...
        assert_eq!(get_u64(&node2, super::RIGHT_CHILD_FIELD_OFFS),
            super::get_entry_value(&node1, orig_sep_index));
        assert_eq!(get_u64(&node3, super::RIGHT_CHILD_FIELD_OFFS),
            get_u64(&node1, super::RIGHT_CHILD_FIELD_OFFS));
